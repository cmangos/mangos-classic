...
void Spell::EffectInstaKill(SpellEffectIndex /*eff_idx*/)
{
    ...
    m_spellLog.AddLog(SPELL_EFFECT_INSTAKILL, unitTarget->GetObjectGuid());
...
void Spell::EffectDummy(SpellEffectIndex eff_idx)
{
    ...
    // TODO: Why do we have to test all possibility to get correct target?
    if (unitTarget)
        m_spellLog.AddDummy(unitTarget->GetObjectGuid());
    else if (gameObjTarget->GetObjectGuid())
        m_spellLog.AddDummy(gameObjTarget->GetObjectGuid());
    else if (itemTarget)
        m_spellLog.AddDummy(itemTarget->GetObjectGuid());
...
void Spell::EffectPowerDrain(SpellEffectIndex eff_idx)
{
    ...
    m_spellLog.AddPowerDrain(unitTarget->GetObjectGuid(), new_damage, powerType, gainMultiplier);
...
void Spell::EffectCreateItem(SpellEffectIndex eff_idx)
{
    if (DoCreateItem(eff_idx, m_spellInfo->EffectItemType[eff_idx]))
        m_spellLog.AddCreateItem(m_spellInfo->EffectItemType[eff_idx]);
}
...
void Spell::EffectOpenLock(SpellEffectIndex eff_idx)
{
    ...
    if (gameObjTarget)
    {
        ...
        m_spellLog.AddOpenLockGObj(*guid);
    }
    else if (itemTarget)
    {
        lockId = itemTarget->GetProto()->LockID;
        guid = &itemTarget->GetObjectGuid();
        m_spellLog.AddOpenLockItem(*guid);
    }
    else
    {
...
void Spell::EffectInterruptCast(SpellEffectIndex /*eff_idx*/)
{
    ...
    if (interruptedSpellId)
        m_spellLog.AddInterruptCast(unitTarget->GetObjectGuid(), interruptedSpellId);
}
...
void Spell::EffectFeedPet(SpellEffectIndex eff_idx)
{
    ...
    m_spellLog.AddFeedPet(foodItem->GetEntry());
...
void Spell::EffectDismissPet(SpellEffectIndex /*eff_idx*/)
{
    ...
    m_spellLog.AddDismissPet(pet->GetObjectGuid());
...
void Spell::EffectAddExtraAttacks(SpellEffectIndex /*eff_idx*/)
{
    ...
    m_spellLog.AddExtraAttack(unitTarget->GetObjectGuid(), damage);
}
...
void Spell::EffectSummonCritter(SpellEffectIndex eff_idx)
{
    ...
    critter->SetUInt32Value(UNIT_FIELD_FLAGS, (UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_FLAG_PASSIVE | info->UnitFlags));
...
void Spell::EffectDurabilityDamage(SpellEffectIndex eff_idx)
{
    ...
    if (slot < 0)
    {
        ...
        m_spellLog.AddDurability(unitTarget->GetObjectGuid());
        return;
    }
    ...
    {
        m_spellLog.AddDurability(unitTarget->GetObjectGuid(), item->GetEntry(), damage);
