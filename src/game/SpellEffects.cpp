    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                {
                    m_caster->CastCustomSpell(m_caster, 12976, &int32(m_caster->GetMaxHealth() * 0.3), nullptr, nullptr, true, nullptr);
                    return;
                }
                ...
                {
                    if (!(uint32 rand = urand(0, 5)))       // Transporter Malfunction - 1/6 polymorph
                        m_caster->CastSpell(m_caster, 23444, true);
                    else if (rand <= 4)                     // Evil Twin               - 4/6 evil twin
                ...
                {
                    if (!(uint32 rand = urand(0, 11)))      // 1/12 fire
                        m_caster->CastSpell(m_caster, 23449, true);
                    else if (rand <= 3)                     // 4/12 evil twin
                        m_caster->CastSpell(m_caster, 23445, true);
                ...
                case 24930:                                 // Hallow's End Treat
                {
                    ...
                    const uint32 spell_list[3] = {24924, 24925, 24926, 24927};
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 28332:
                {
                    ...
                    if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER)
                        ((Player*)unitTarget)->AddSpellMod(SpellModifier(SPELLMOD_RESIST_MISS_CHANCE, SPELLMOD_FLAT, damage, m_spellInfo->Id, uint64(0x0000000000000100)), true);
