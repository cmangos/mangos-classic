...
void UnitAI::OnSpellCastStateChange(Spell const* spell, bool state, WorldObject* target)
{
    ...
    if (spellInfo->HasAttribute(SPELL_ATTR_EX4_CAN_CAST_WHILE_CASTING) || spellInfo->HasAttribute(SPELL_ATTR_ON_NEXT_PLAYER_SWING) || spellInfo->HasAttribute(SPELL_ATTR_ON_NEXT_NPC_SWING))
        return;
