...
void Spell::update(uint32 difftime)
{
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER && m_timer != 0) &&
            ...
            (m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK || !((Player*)m_caster)->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLINGFAR)))
    {
        ...
        case SPELL_STATE_CASTING:
        {
            if (m_timer > 0)
            {
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    if (((Player*)m_caster)->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING))
                        cancel();
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (((Player*)m_caster)->isMoving())
        {
            ...
            if ((!((Player*)m_caster)->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLINGFAR) || m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK) &&
