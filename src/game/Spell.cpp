                    ...
                    case 0:
                        ...
                        if(m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && m_spellInfo->SpellFamilyFlags & UI64LIT(0x00000800))
                        {
                            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                                if (Unit *target = ObjectAccessor::Instance().GetUnit(*m_caster, ((Player*)m_caster)->GetSelection()))
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch(targetMode)
    {
        ...
        case TARGET_ALL_FRIENDLY_UNITS_AROUND_CASTER:
            FillAreaTargets(targetUnitMap,m_caster->GetPositionX(), m_caster->GetPositionY(),radius,PUSH_SELF_CENTER,SPELL_TARGETS_FRIENDLY);
            break;
        ...
        case TARGET_ALL_ENEMY_IN_AREA_CHANNELED:
            ...
            if (m_spellInfo->Effect[effIndex]!=SPELL_EFFECT_PERSISTENT_AREA_AURA)
                FillAreaTargets(targetUnitMap,m_caster->GetPositionX(), m_caster->GetPositionY(),radius,PUSH_DEST_CENTER,SPELL_TARGETS_AOE_DAMAGE);
            break;
        case TARGET_MINION:
...
bool Spell::CanAutoCast(Unit* target)
{
    ...
    for(int j = 0; j < MAX_EFFECT_INDEX; ++j)
    {
        if(m_spellInfo->Effect[j] == SPELL_EFFECT_APPLY_AURA)
        {
            ...
            {
                if( target->GetAuras().count(Unit::spellEffectPair(m_spellInfo->Id, SpellEffectIndex(j))) >= m_spellInfo->StackAmount)
                    return false;
            }
        }
