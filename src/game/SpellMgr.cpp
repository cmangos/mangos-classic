...
uint32 GetSpellCastTime(SpellEntry const* spellInfo, Spell const* spell)
{
    ...
    if (spellInfo->HasAttribute(SPELL_ATTR_RANGED) && (!spell || !spell->IsAutoRepeat()) && spellInfo->SpellFamilyFlags & 0x0000000000001000) // Multishot only (Alternativ CastingTimeIndex = 3 setzen)
        castTime = 500;
