...
{
    QueryResult* result = WorldDatabase.Query("SELECT Class, Level, BaseHealthExp0, BaseMana, "
        "BaseDamageExp0, BaseMeleeAttackPower, BaseRangedAttackPower, BaseArmor "
    ...
    do
    {
        ...
        if ((Level == 0) || (Level > MAX_CREATURE_LEVEL_CLASSIC))
        {
        ...
        cCLSExp0.BaseMana               = fields[3].GetUInt32();
        cCLSExp0.BaseDamage             = fields[4].GetFloat();
        cCLSExp0.BaseMeleeAttackPower   = fields[5].GetFloat();
        cCLSExp0.BaseRangedAttackPower  = fields[6].GetFloat();
        cCLSExp0.BaseArmor              = fields[7].GetUInt32();

        m_creatureClassLvlStats[classArrayIndex][Level] = cCLSExp0;
        ++storedRow;
    ...
    sLog.outString();
    if (storedRow == (MAX_CREATURE_LEVEL_CLASSIC * MAX_CREATURE_CLASS))
        sLog.outString(">> Found %u creature stats definitions.", storedRow);
    else
        sLog.outErrorDb("Found only %u valids row when expected %u! Some creature will not have valid data!", storedRow, uint32(MAX_CREATURE_LEVEL_CLASSIC * MAX_CREATURE_CLASS));
}

CreatureClassLvlStats const* ObjectMgr::GetCreatureClassLvlStats(uint32 creatureClass, uint32 level) const
{
    if ((level == 0) || (level > MAX_CREATURE_LEVEL_CLASSIC))
    {
        sLog.outError("CreatureClassLvlStats: Incorrrect class or level recieved (class = [%u], level = [%u]).", creatureClass, level);
        return &m_creatureClassLvlStats[0][0];                  // return dummy data (creature will not have correct value)
    }
    ...
    return &m_creatureClassLvlStats[classArrayIndex][level];
}
