...
void Aura::HandleModStealth(bool apply, bool Real)
{
    if (apply)
    {
        ...
        if (Real)
        {
            m_target->SetByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAGS_CREEP);
            if(m_target->GetTypeId()==TYPEID_PLAYER)
    ...
    else
    {
        ...
        if (Real && !m_target->HasAuraType(SPELL_AURA_MOD_STEALTH))
        {
            ...
            if (m_target->GetVisibility()!=VISIBILITY_OFF)
            {
                m_target->RemoveByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAGS_CREEP);
                if(m_target->GetTypeId()==TYPEID_PLAYER)
    ...
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            if(m_modifier.m_auraname == SPELL_AURA_PERIODIC_DAMAGE)
            {
                ...
                pdamage = pCaster->SpellDamageBonus(target, spellProto, pdamage, DOT, GetStackAmount());
        ...
        case SPELL_AURA_PERIODIC_HEALTH_FUNNEL:
        {
            ...
            uint32 heal = pCaster->SpellHealingBonus(pCaster, spellProto, uint32(new_damage * multiplier), DOT);
