...
bool InstanceMap::Add(Player *player)
{
...

    {
        ...
        if(IsDungeon())
        {
            ...
            if(!mapSave)
            {
                DETAIL_LOG("InstanceMap::Add: creating instance save for map %d with instance id %d", GetId(), GetInstanceId());
                mapSave = sInstanceSaveMgr.AddInstanceSave(GetId(), GetInstanceId(), 0, true);
            }
